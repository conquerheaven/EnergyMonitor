namespace EnergyMonitor.Controllers.Admin.ReportsTemplate
{
    ...
    /// </summary>
    public partial class AreaWaterMonth : Telerik.Reporting.Report
    {
        public AreaWaterMonth()
        {
            //
        ...
        {
            var areaIdObj = ReportParameters["areaId"].Value;
            var yearObj = ReportParameters["year"].Value;
            if (areaIdObj != null)
            {
                int areaId = Convert.ToInt32(areaIdObj);
                int year = 0;
                if (yearObj != null)
                {
                ...
                ISchoolAreaRepos areaRepos = new SchoolAreaRepos();
                var area = areaRepos.GetSchoolArea(areaId);
                if (area != null)
                {
                    string buildingName = area.SAI_Name;
                    ...
                    var monthDataDic = analogHistoryRepos.GetEnergyStatisMonth(2, areaId, startDate, endDate2, waterPowers, 1).ToDictionary(x => x.Time.Month, x => x.StatisVal);
